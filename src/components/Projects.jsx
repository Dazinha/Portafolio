...
const ProjectCard = ({ project, index, itemVariants, t }) => {
  ...
  return (
    ...
    >
      {/* Imagen Principal */}
      <div className="relative w-full lg:w-[45%] flex-shrink-0 bg-slate-800 overflow-hidden min-h-[300px] lg:min-h-full">
        {images.length > 0 ? (
          <div 
        ...
        {images.length > 1 && (
          <>
            <button 
              onClick={prevImg} 
              className="absolute left-3 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-indigo-500 border border-white/10 text-white rounded-full p-2 opacity-0 group-hover:opacity-100 transition-all duration-300 z-20 backdrop-blur-sm shadow-lg hover:shadow-indigo-500/40 hover:scale-110" 
            ...
            <button 
              onClick={nextImg} 
              className="absolute right-3 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-indigo-500 border border-white/10 text-white rounded-full p-2 opacity-0 group-hover:opacity-100 transition-all duration-300 z-20 backdrop-blur-sm shadow-lg hover:shadow-indigo-500/40 hover:scale-110" 
            ...
            {/* Dots */}
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 z-20 bg-black/30 px-2 py-1.5 rounded-full backdrop-blur-md border border-white/5 opacity-0 group-hover:opacity-100 transition-opacity">
              {images.map((_, i) => (
                <div 
                  key={i} 
        ...
        {images.length > 0 && (
          <button 
            ...
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-14 h-14 bg-black/40 hover:bg-indigo-500 border border-white/20 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 z-30 shadow-xl backdrop-blur-md hover:scale-110"
